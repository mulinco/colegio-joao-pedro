...
test.describe('Navegação Principal - Colégio João Pedro', () => {
  ...
  test('1. deve levar para a seção de contato ao clicar em Agendar Visita', async ({ page }) => {
    ...
    const btnAgendar = page.getByRole('link', { name: /agendar visita/i }).first();
  ...
  test('2. deve abrir a página de proposta pedagógica', async ({ page }) => {
    ...
    const btnProposta = page.getByRole('link', { name: /conhecer a escola/i }).first();
  ...
  test('3. deve abrir o modal de confirmação do WhatsApp', async ({ page }) => {
    ...
    const confirmBtn = modalContainer.getByRole('button', { name: /Sim, iniciar agora/i });
